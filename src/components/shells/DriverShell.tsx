...
const driverNav = [
  { href: "/driver", label: "Tổng quan", icon: "chart" },
  { href: "/stations", label: "Sạc ngay", icon: "bolt" },
  { href: "/sessions", label: "Đang sạc", icon: "plug" },
  { href: "/driver/vehicles", label: "Xe", icon: "car" },
  { href: "/driver/earnings", label: "Doanh thu", icon: "trending" },
];
...
export default function DriverShell({ children, title, user }: { children: React.ReactNode; title?: string; user: any }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      {/* TOP HEADER - Brand gradient (Xanh SM) */}
          ...
          <div className="flex items-center gap-2">
            {stats && (
              <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white/15 backdrop-blur rounded-full text-sm font-bold">
                <Icon name="wallet" className="w-4 h-4" /> {stats.balance.toLocaleString("vi-VN")} ₫
              </div>
            )}
                ...
                <>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-56 z-20 rounded-xl shadow-xl overflow-hidden animate-fadeIn" style={{ background: "var(--card-bg)", border: "1px solid var(--border)", color: "var(--text)" }}>
                    <div className="p-3" style={{ background: "var(--accent-soft)" }}>
                      ...
                      <p className="text-xs text-muted">Tài xế Xanh SM</p>
                      <span className="badge-purple mt-1 inline-block">⭐ {user?.loyaltyTier || "BRONZE"}</span>
                    </div>
                    <Link href="/profile" onClick={() => setShowMenu(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-800"><Icon name="settings" className="w-4 h-4" /> Tài khoản</Link>
                    <Link href="/wallet" onClick={() => setShowMenu(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-800"><Icon name="wallet" className="w-4 h-4" /> Ví</Link>
                    <Link href="/invoices" onClick={() => setShowMenu(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-800"><Icon name="receipt" className="w-4 h-4" /> Hoá đơn</Link>
                    <button onClick={logout} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 border-t" style={{ borderColor: "var(--border)" }}><Icon name="logout" className="w-4 h-4" /> Đăng xuất</button>
                  </div>
                </>
              )}
            </div>
